...
import { fetchCurrentUser, verifyCurrentUser } from '../services/user';
...
test("Handle username change", async () => {
    ...
    const editButton = await screen.findByRole('button', { name: /edit username/i });
...
test("Shows error on invalid username", async () => {
    ...
    const errorDiv = await screen.findByText(/Username must be 3-50 characters/i, { selector: '[aria-live="assertive"]' });
...
test("Handles API error on username update", async () => {
    ...
    const errorDiv = await screen.findByText(/Failed to update username/i, { selector: '[aria-live="assertive"]' });
    expect(errorDiv).toBeInTheDocument();
});